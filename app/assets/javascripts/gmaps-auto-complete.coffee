class GmapsCompleter
  ...
  init: (opts) ->
    ...
    @map.fitBounds(bounds) if @map && lat != 0 && lng != 0
  ...
  autoCompleteInit: (opts) ->
    ...
    defaultAutocompleteOpts = 
      ...
      select: (event,ui) ->
        $.ajax
          url: '/app_helpers/api_json_lookup',
          data: {
            type: 'placeid',
            value: ui.item.geocode},
          success: (data)->
            geometry = data.result.geometry
            self.updateUI  ui.item.value, geometry.location
            self.updateMap geometry
...
class GmapsCompleterDefaultAssist
  ...
  updateMap: (geometry) ->
    ...
    if geometry.viewport
      bounds = new google.maps.LatLngBounds(
        new google.maps.LatLng(geometry.viewport.southwest.lat, geometry.viewport.southwest.lng),
        new google.maps.LatLng(geometry.viewport.northeast.lat, geometry.viewport.northeast.lng)
      )
      map.fitBounds(bounds) if map

    else if geometry.location  
      bounds = new google.maps.LatLngBounds(
        new google.maps.LatLng(geometry.location.lat, geometry.location.lng),
        new google.maps.LatLng(geometry.location.lat, geometry.location.lng)
      )
      map.fitBounds(bounds) if map
  ...
  positionOutputter: (latLng) ->
    # $('input.form-control.gmaps-input-latitude').val latLng.lat
    # $('input.form-control.gmaps-input-longitude').val latLng.lng 
