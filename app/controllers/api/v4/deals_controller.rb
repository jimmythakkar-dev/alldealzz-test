class Api::V4::DealsController < Api::V4::BaseController
  ...
  def booked_deals
    used_deals = if params[:deal_type] == "paid"
      current_user.end_user_used_deals.joins(:transaction_detail).joins(:deal).where("transaction_details.payment_status = ? AND deals.deal_type  in (?)",TransactionDetail::PaymentStatus[:success], [Deal::DealType[:pd], Deal::DealType[:lmd]]).group("end_user_used_deals.id")
    else
      current_user.end_user_used_deals.joins(:deal).where("deals.deal_type not in (?)", [Deal::DealType[:pd], Deal::DealType[:lmd]]).group("end_user_used_deals.id")
    end
