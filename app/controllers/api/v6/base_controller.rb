class Api::V6::BaseController < ActionController::Base
  ...
  def default_v3_api_opts( opts = {})
    opts.merge({v3_api: true})
  end
  ...
  def current_user_city
    (current_user.city_id || City.find_by_name(params[:city]).try(:id)) rescue 1
  end
  # -------------------------------------------------
end
