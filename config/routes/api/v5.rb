namespace :v5 do
  ...
  post 'end_user/favourites', to: "end_users#favourites"
  post 'end_user/points', to: "end_users#points"
  post 'end_user/points_history', to: "end_users#points_history"

  post 'deals', to: "deals#index"
  post 'deals/:id', to: "deals#show"
  post 'deals/:id/reviews', to: "deals#reviews"
  post 'deals/:id/my_review', to: "deals#my_review"
  ...
  resources :deals, only: [] do
    resources :transaction_details, only: [] do
      ...
      post :create_charge, on: :collection
    end
  end
  ...
  resources :last_minute_deals, only: [] do
    post :index, on: :collection
    post :expired_deals, on: :collection
  ...
  resources :end_user_applied_codes, only: [] do
    post :index, on: :collection
    post :apply_code, on: :collection
  end
  resources :cashbacks, only: [] do
    post :index, on: :collection
    post :deal_cashback, on: :collection
    post :trending_cashbacks, on: :collection
    post :category_cashbacks, on: :collection
  end
  ...
  resources :deal_categories, only: [] do
    post :index, on: :collection
  end
end